            ...
            */
            ...
            struct weighted_density_impl
                ...
                weighted_density_impl(Args const &args)
                        : cache_size(args[density_cache_size]), cache(cache_size), num_bins(args[density_num_bins]),
                          samples_in_bin(num_bins + 2, 0.), bin_positions(num_bins + 2), histogram(
                ...
                void operator()(Args const &args) {
                    ...
                    if (cnt == this->cache_size) {
                        ...
                        for (typename histogram_type::const_iterator iter = this->cache.begin();
                             iter != this->cache.end(); ++iter) {
                    ...
                    if (this->is_dirty) {
                        ...
                        for (std::size_t i = 0; i < this->num_bins + 2; ++i) {
                            this->histogram[i] = std::make_pair(this->bin_positions[i],
                                                                numeric::fdiv(this->samples_in_bin[i],
                                                                              sum_of_weights(args)));
                        }
                    }
...
//
        namespace tag {
            struct weighted_density
                    : depends_on<count, sum_of_weights, min, max>, density_cache_size, density_num_bins {
                /// INTERNAL ONLY
                ///
