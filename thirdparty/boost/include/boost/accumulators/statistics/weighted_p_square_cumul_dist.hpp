            ...
            */
            ...
            struct weighted_p_square_cumulative_distribution_impl
                ...
                weighted_p_square_cumulative_distribution_impl(Args const &args)
                        : num_cells(args[p_square_cumulative_distribution_num_cells]), heights(num_cells + 1),
                          actual_positions(num_cells + 1), desired_positions(num_cells + 1), histogram(num_cells + 1),
                          is_dirty(true) {
                }
                    ...
                    if (cnt <= b + 1) {
                        ...
                        for (std::size_t i = 1; i < b + 1; ++i) {
                            this->desired_positions[i] = this->actual_positions[0]
                                                         + numeric::fdiv(
                                    (i - 1) * (sum_of_weights(args) - this->actual_positions[0]), b);
                        }
                        ...
                        for (std::size_t i = 1; i < b; ++i) {
                            ...
                            if ((d >= 1. && dp > 1.) || (d <= -1. && dm < -1.)) {
                                ...
                                float_type h = this->heights[i] +
                                               sign_d / (dp - dm) * ((sign_d - dm) * hp + (dp - sign_d) * hm);
                ...
                result_type result(Args const &args) const {
                    if (this->is_dirty) {
                        ...
                        for (std::size_t i = 0; i < this->histogram.size(); ++i) {
                            this->histogram[i] = std::make_pair(this->heights[i],
                                                                numeric::fdiv(this->actual_positions[i],
                                                                              sum_of_weights(args)));
                        }
                    }
...
//
        namespace tag {
            struct weighted_p_square_cumulative_distribution
                    : depends_on<count, sum_of_weights>, p_square_cumulative_distribution_num_cells {
